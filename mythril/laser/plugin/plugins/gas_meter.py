...
def get_function_gas_meter_annotation(state: GlobalState) -> FunctionGasMeterAnnotation:
    ...
    annotations = cast(
        List[FunctionGasMeterAnnotation], list(state.get_annotations(FunctionGasMeterAnnotation))
    )
    ...
    if len(annotations) == 0:
        ...
        try:
            ...
            annotation = FunctionGasMeterAnnotation()
...
class FunctionGasMeterBuilder(PluginBuilder):
    name = "function-gas-meter"

    def __call__(self, *args, **kwargs):
        return FunctionGasMeter()

class FunctionGasMeter(LaserPlugin):
    ...
    def acc_mem_gas(self, path: List[int], target_location: object) -> None:
    ...
    def initialize(self, symbolic_vm: LaserEVM) -> None:
        ...
        def add_opcode_gas_hook(state: GlobalState):
            annotation = get_function_gas_meter_annotation(state)
          
            opcode = state.instruction["opcode"]
            
            gas_meter = state.mstate.pc_gas_meter.get(pc, InstrGasMeter())
